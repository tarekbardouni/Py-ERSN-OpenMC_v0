...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        uic.loadUi("GUI_Install.ui", self)
    ...
    def initUI(self):
        ...
        self.menu_bar()
        self.define_pB()
        self.tab_install.currentChanged.connect(self.set_Options_to_default) #changed!
    ...
    def define_pB(self):
        ...
        self.pB_exit.clicked.connect(exit)
    ...
    def get_XS_data(self, library):
        ...
        self.script_exec('bash get_cross_sections.sh ' + OPTIONS)
    ...
    def get_NJOY_build(self):
        ...
        self.script_exec('bash get_njoy21.sh ' + WORK_DIR_XS + ' ' + NJOY_RELEASE)
    ...
    def set_Options_to_default(self):
        ...
        if tab_index == 0:                     # miniconda tab
            ...
            self.rB_py38_prerequis.toggled.connect(lambda: self.PyVer_btnstate(self.rB_py38_prerequis, ENV_NAME_PREFIX, self.lineEdit_Env_Name_prerequis))
            self.rB_yes_all_prerequis.setChecked(True)
            ...
            self.rB_py38.toggled.connect(lambda: self.PyVer_btnstate(self.rB_py38, ENV_NAME_PREFIX, self.lineEdit_Env_Name))
    ...
    def Proc_Start(self):
        ...
        if "miniconda3" in CONDA:
            ...
            self.script_exec('bash openmc-conda-install.sh ' + OPTIONS)
        else:
            self.print_lines("can't continue without miniconda3 installed !")
            return
    ...
    def update_prerequis_options(self):
        ...
        UPDATE_CONDA = 'no'
        INSTALL_OPENMC = 'no'
        self.Test_If_rB_Checked(self.Python_Version_gB_prerequis, self.rB_py37_prerequis, self.rB_py38_prerequis)
    ...
    def update_openmc_options(self):
        ...
        INSTALL_OPENMC = 'yes'
        self.Test_If_rB_Checked(self.Python_Version_gB, self.rB_py37, self.rB_py38)
    ...
    def print_lines(self, text):
        import textwrap
        rA_width = int(self.receiveArea.width() / 5)   # 5 could be changed
        print('#' * int(rA_width / 2.09))              # 2.04 to convert pixels to character
        lines = textwrap.wrap(text, fix_sentence_endings = True)
        print("\n".join(line.center(rA_width) for line in lines))
        print('#' * int(rA_width / 2.09))

    def Test_If_rB_Checked(self, gB, rB1, rB2):
        global rc
    ...
    def OpenFiles(self):
        ...
        # self.highlight = xml_syntax.XMLHighlighter(self.receiveArea.document())
